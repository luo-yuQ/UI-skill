# Material Language

## Analyze

Record tangible object materials and surface appearance, such as metal, stone, wood, leather, fabric, glass, crystal, ceramic, paper, or polished, matte, worn, rough, smooth, reflective, and subdued surfaces.

## Direct observation

Highlights, texture, translucency, surface wear, and edge behavior are observable. A named material may be inferred when those cues support it.

## Describe

Pair a material name with short qualities, for example "silver metal: hard, reflective, clean." Lower confidence or use uncertainty when the substance cannot be distinguished reliably.

## Common errors

- Treating color alone as proof of a material.
- Calling every reflective surface metal or every translucent surface glass.
- Listing fire, smoke, fog, bloom, glow, magical light, particles, or sparks as materials. Record them under lighting, decoration/effects, atmospheric description, or `visual_description` instead.
- Turning material observations into implementation or asset-production advice.

## Do not analyze

Do not prescribe shaders, texture maps, engine settings, or materials for future UI elements. Atmospheric effects, light effects, and particles are not materials.
