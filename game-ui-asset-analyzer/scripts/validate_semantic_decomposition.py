#!/usr/bin/env python3
"""Validate a Stage2-A semantic_decompose v0.1 VLM document."""
...
SCHEMA_PATH = ROOT / "schemas" / "semantic-decomposition.schema.json"
...
def _declared_analysis_size(data: Any) -> tuple[int, int] | None:
    if not isinstance(data, dict):
        return None
    value = data.get("analysis_image_size")
...
def validate_document(data: Any, analysis_image: Path) -> list[str]:
    """Validate schema, IDs, actual image size, and child bbox bounds without mutation."""

    errors = validate_schema(data)
    errors.extend(_validate_child_ids(data.get("children") if isinstance(data, dict) else None))
    ...
    declared_size = _declared_analysis_size(data)
    if declared_size is not None:
        if declared_size != actual_size:
            errors.append(
                "$.analysis_image_size: declared "
                f"{declared_size[0]}x{declared_size[1]} does not match actual "
                f"Analysis Image {actual_size[0]}x{actual_size[1]}"
            )
    errors.extend(
        _validate_bbox_bounds(
            data.get("children") if isinstance(data, dict) else None,
            actual_size,
        )
    )
    return errors
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate Stage2-A semantic_decompose v0.1 JSON."
    )
    parser.add_argument("document", type=Path, help="semantic-decomposition.json")
    parser.add_argument(
        "--analysis-image",
        required=True,
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        ...
        return 1
    print(f"Valid semantic decomposition v0.1: {args.document}")
    return 0
