#!/usr/bin/env python3
"""Asset Admission (Stage2-A2): VLM KEEP/DROP gate over A1 candidates.

Production chain position:

    direct-assets.json (A1 Direct Asset Discovery, immutable)
      -> Asset Admission (this module)
...
- ``--image``: the Analysis Image matching the candidates document's
  ``analysis_image_size`` (verified before any VLM call).
- ``--candidates-json``: one A1 ``direct-assets.json`` document.
- ``--output-dir``: run directory (one ``run-NNN`` subdir per run).
- ``--model``: optional model override of ``STAGE2A_VLM_MODEL``.
- ``--runs``: number of independent admission calls (default 1).

Output contract (per run directory, new files only — the input candidates
JSON is never modified):

- ``input-candidates.json``: verbatim copy of the A1 document (evidence).
- ``admission-decisions.json``: frozen decision list
  (``asset-admission-v0.1``; one decision per candidate id, frozen reason
  codes, no bbox/taxonomy/label output fields).
- ``accepted-assets.json``: KEEP candidates materialized by joining A1 fields
  with admission decisions (``asset-admission-v0.1``).
- ``raw-provider-response.json``: raw provider envelope evidence.
- ``run-summary.json`` / ``summary.json``: counts + provider usage.
"""
...
def create_default_client() -> ChatCompletionsVLMClient:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Asset admission failed: {exc}", file=sys.stderr)
        return 1
    counts = ", ".join(
