...
Production chain position (frozen Human Review contract):

    direct-assets.json            (immutable VLM output, never edited)
          +
    review-overrides.json         (human corrections, separate file)
      -> apply_direct_asset_review (this module, deterministic)
      -> reviewed-direct-assets.json
      -> Stage2-C build_repair_relations / prepare_repair_inputs
      -> Stage2-E recomposition

Hard rules (frozen):

- ``direct-assets.json`` is immutable VLM output; this module only reads it.
- All human corrections live in ``review-overrides.json``
  (``direct-asset-review-overrides-v0.1``): per-asset ``bbox`` / ``decision``
  overrides plus ``manual_assets`` additions.
- Overrides are validated fail-fast: unknown asset ids, unknown fields,
  non-integer or out-of-bounds bboxes, image-size mismatches, and basename
  mismatches are all hard errors. No bbox scaling is ever performed.
- The output ``reviewed-direct-assets.json`` (``direct-assets-reviewed-v0.1``)
  records provenance (source files, schema versions) and a
  ``review_summary``; dropped originals are removed, modified bboxes keep
  ``review.original_bbox_source``, manual assets get
  ``review_origin: "manual"``.
- The output is written atomically and must not overwrite either input.

This module is deterministic: no VLM, no network, no randomness.
"""
