#!/usr/bin/env python3
"""Direct Asset Discovery (Stage2-A1): one direct, non-recursive terminal-asset
census on one clean UI image.

Production chain position:

    Clean UI
      -> Direct Asset Discovery (this module)
      -> direct-assets.json + overlay images + provider diagnostics
      -> Asset Admission (asset_admission.py)
      -> Human Review Overrides (review-overrides.json)
      -> Deterministic Apply (apply_direct_asset_review.py)
      -> reviewed-direct-assets.json
      -> Stage2-B / Stage2-C

Input contract (CLI):

- ``--image``: one clean UI image (the Stage0 text-cleaned output).
- ``--output-dir``: run directory; ``source.png`` and ``analysis-image.png``
  are written there (source is copied unchanged, analysis is the deterministic
  downscaled model input from ``prepare_analysis_input``).
- ``--model``: optional model override of ``STAGE2A_VLM_MODEL``.
- ``--runs``: number of independent census calls (default 1).

Output contract (per run directory):

- ``direct-assets.json``: immutable VLM census output; ``schema_version``
  ``0.1``; assets carry ``bbox_analysis`` (analysis-image pixels) plus a
  deterministic ``bbox_source`` mapping back to source pixels. Never edited by
  hand or by later stages.
- ``overlay-analysis.png`` / ``overlay-source.png``: review overlays in both
  coordinate spaces.
- ``raw-response.json``: raw provider envelope evidence (decoded from the
  client when exposed), including truncation evidence saved before a
  ``VLMResponseTruncatedError`` propagates.
- ``run-metadata.json``: per-run metadata; ``summary.json``: run summary.

VLM client: Chat Completions production contract via
``chat_completions_client.ChatCompletionsVLMClient`` (``thinking`` omitted,
API-level strict JSON Schema, ``max_tokens=12000``).
"""
...
from chat_completions_client import (  # noqa: E402
...
def build_response_schema(
    ...
    """Build the response schema from the frozen taxonomy values."""
...
The exclusion of text glyphs and numerals overrides any general taxonomy note
about preserving special text artwork. Do not emit a text asset. Do not turn
taxonomy containment examples into a component hierarchy.
...
def create_default_client(model: str) -> ChatCompletionsVLMClient:
    """Build the production Chat Completions client from the environment."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run a direct, non-recursive terminal-asset census on a clean UI "
            "(Stage2-A1 Direct Asset Discovery)."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Direct asset discovery failed: {exc}", file=sys.stderr)
        return 1
    counts = ", ".join(
