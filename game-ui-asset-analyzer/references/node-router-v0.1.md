# Stage2-A Node Router v0.1

Status: **contract implemented / behavior validated**.
...
You are performing Stage2-A Node Router v0.1 on exactly one Current Node Analysis Image.
...
- `component_instance`: the Current Node is one concrete, self-contained UI/business component, and its next natural decomposition is into visual assets.
- `structural_group`: the Current Node's next natural Direct Children are multiple stable regions with different responsibilities or semantics, rather than instances of one business object.
...
3. If it is one concrete self-contained component whose next split is visual assets, choose `component_instance`.
4. If it can be stably split into natural regions with different responsibilities, choose `structural_group`.
...
`resolve_node_action(role)` raises an error for every role outside the frozen enum. The VLM never emits or selects `next_action`. Router v0.1 does not execute any mapped action.
...
Node Router v0.1 classifies only. `structural_split` and `expand_instances` remain prototypes; the recursive engine is not implemented. This contract does not modify or execute frozen `semantic_decompose` v0.1, its ten-category taxonomy, Stage2-B or later stages, extraction, FairyGUI, or XML.
