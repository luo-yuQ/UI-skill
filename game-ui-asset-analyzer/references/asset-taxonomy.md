...
| `icon` | A localized UI visual representing a discrete object, item, action, status, resource, ability, category, or symbolic concept. It may be highly rendered, glowing, decorated, or relatively large, such as an avatar portrait; artwork complexity and bbox size do not turn it into an illustration. Containment inside a panel, button, slot, card, or other container does not remove its independent semantic identity. |
| `illustration` | An artwork-like visual region whose primary role is decorative, narrative, scenic, promotional, or content presentation rather than representing one localized UI symbol or object. It may contain a scene or multiple subjects and need not cross a fixed size threshold. Classify by its role in the UI, not rendering complexity alone. |
