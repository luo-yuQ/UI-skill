...
`analysis_image_size` is authoritative runtime metadata. The model may emit the field to satisfy a frozen schema, but `ProductionVisualAdapter` canonicalizes it from the actual Analysis Image before validation; semantic fields and bboxes remain untouched, and bbox bounds are still validated against the same real image dimensions.
...
All child Node Crops are cropped from the parent's existing `node-crop.png`, never from the resized Analysis Image or the original source image. Recursive children from `structural_split` and `expand_instances` additionally receive Analysis Images generated by the shared `prepare_analysis_input.prepare_analysis_input(..., max_width=1024, force_width=True)` utility, preserving proportional rounded height.
...
For `semantic_decompose` decision `decompose`, each valid visual child becomes an `asset` Node Record with both parent-coordinate bboxes, `terminal: true`, `next_action: stop`, and `status: done`. The Runtime materializes its `node-crop.png` directly from the parent Node Crop using `bbox_in_parent_crop`; it receives no Analysis Image and is never queued. For `stop_as_asset`, the current component instance itself becomes the asset; no duplicate child is created and its existing Node Crop is preserved.
...
`-- nodes/<node-id>/
    |-- node.json
    |-- node-crop.png             # every node, including terminal asset children
