...
description: Classify and recursively schedule Stage2-A UI nodes with the Node Router v0.1 and Recursive Runtime v0.1 contracts, split structural groups into stable direct regions, expand repeated groups into peer component instances, decompose selected components into reusable visual asset candidates, deterministically resolve terminal asset/stop state, assign the v0.3 four-state extraction strategy contract, map candidate bounding boxes back into crop pixels, and support interactive file-based or automated Responses API visual execution. Use for Stage2-A routing, level-by-level recursive traversal, structural splitting, repeated-instance expansion, terminal-state resolution, UI asset decomposition, mechanics validation, and real-image execution; do not enter Stage2-B asset PNG extraction, segmentation models, alpha matting, pixel generation, engine assembly, or final PNG manifests.
---
...
When the resolved action is `semantic_decompose`, use [references/semantic-decompose-v0.1.md](references/semantic-decompose-v0.1.md), emit `schemas/semantic-decomposition.schema.json`, and run `scripts/validate_semantic_decomposition.py` against the actual Analysis Image. The v0.1.2 scoped contract identifies direct foundational UI component children by visual composition, classifies `icon` versus `illustration` by UI component role, and treats `node_id` as caller-owned while retaining the v0.1 output schema and taxonomy enum; do not change its taxonomy enum, schema, or component-composition decision boundary.
...
For a manual real-image run, use `scripts/run_recursive_runtime.py --adapter interactive --validation-mode real_image`. When it returns `WAITING_FOR_ADAPTER`, inspect the reported Analysis Image, write the matching response envelope under `adapter-responses/`, and invoke the same entrypoint with `--adapter interactive --resume`. For automated execution, configure `STAGE2A_VLM_BASE_URL`, `STAGE2A_VLM_API_KEY`, `STAGE2A_VLM_MODEL`, and optional `STAGE2A_VLM_TIMEOUT`, then select `--adapter production`. Production uses the verified `/v1/responses` contract and fails closed; it never falls back to Fake or Interactive. Never hand-edit queues, tree state, child records, transforms, or deferred decisions.

Current recursive Stage2-A status: Level-1 Region Decomposition v0.1, Coordinate Contract v0.1, Node Router v0.1, `structural_split` v0.1, `expand_instances` v0.1, and Asset / Stop Contract v0.1 are **FROZEN**. `semantic_decompose` v0.1.2 is **IMPLEMENTED / taxonomy and caller-identity closure** while retaining its v0.1 schema and taxonomy enum. Recursive Runtime v0.1 is **IMPLEMENTED / mechanics validated**; Production Visual Adapter is **IMPLEMENTED**; Responses API VLM Client is **IMPLEMENTED / real API smoke test pending**; real-image R5 is **PENDING**. Runtime is not frozen.
