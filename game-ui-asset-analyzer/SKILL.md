...
When the resolved action is `semantic_decompose`, use [references/semantic-decompose-v0.1.md](references/semantic-decompose-v0.1.md), emit `schemas/semantic-decomposition.schema.json`, and run `scripts/validate_semantic_decomposition.py` against the actual Analysis Image. The v0.1.1 scoped contract identifies direct foundational UI component children by visual composition while retaining the v0.1 output schema and taxonomy; do not change its taxonomy, schema, or component-composition decision boundary.
...
Current recursive Stage2-A status: Level-1 Region Decomposition v0.1, Coordinate Contract v0.1, Node Router v0.1, `structural_split` v0.1, `expand_instances` v0.1, and Asset / Stop Contract v0.1 are **FROZEN**. `semantic_decompose` v0.1.1 is **IMPLEMENTED / component-composition boundary patched** while retaining its v0.1 schema and taxonomy. Recursive Runtime v0.1 is **IMPLEMENTED / mechanics validated**; Production Visual Adapter is **IMPLEMENTED**; Responses API VLM Client is **IMPLEMENTED / real API smoke test pending**; real-image R5 is **PENDING**. Runtime is not frozen.
