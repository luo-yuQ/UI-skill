...
import asset_admission as admission  # noqa: E402
...
class AssetAdmissionContractTests(unittest.TestCase):
    def test_frozen_constants(self):
        self.assertEqual("asset-admission-v0.1", admission.ADMISSION_SCHEMA_VERSION)
        self.assertEqual(("KEEP", "DROP"), admission.DECISION_VALUES)
        self.assertEqual(
            (
        ...
        self.assertEqual(("bbox", "taxonomy", "label"), admission.FORBIDDEN_OUTPUT_FIELDS)
        self.assertEqual(12000, admission.ADMISSION_MAX_TOKENS)

    def test_response_schema_uses_frozen_enums_and_candidate_refs(self):
        ids = ["asset_002", "asset_001"]
        schema = admission.build_admission_response_schema(ids)
    ...
    def test_prompt_defines_a2_task_as_survival_decision(self):
        prompt = admission.build_user_prompt(make_candidates_document(2)["assets"])
    ...
    def test_prompt_sends_full_candidate_manifest_not_only_ids(self):
        ...
        prompt = admission.build_user_prompt([candidate])
        for expected in (
            "asset_026",
    ...
    def test_system_prompt_is_stage2_a2_admission_gate(self):
        self.assertIn("Stage2-A2 Asset Admission Gate", admission.SYSTEM_PROMPT)
        self.assertIn("Do not discover new assets.", admission.SYSTEM_PROMPT)
        self.assertIn(
            "Do not infer candidate meaning from candidate_ref numbering.",
            admission.SYSTEM_PROMPT,
        )
        self.assertIn("bbox_analysis", admission.SYSTEM_PROMPT)
    ...
    def test_response_schema_decisions_exclude_grounding_fields(self):
        schema = admission.build_admission_response_schema(["asset_001"])
...
class AssetAdmissionValidatorTests(unittest.TestCase):
    ...
    def test_thirty_six_inputs_require_exactly_thirty_six_decisions(self):
        decisions = admission.validate_admission_response(
    ...
    def test_missing_candidate_ref_fails(self):
        ...
        with self.assertRaises(ValueError) as context:
            admission.validate_admission_response(response, self.ids)
    ...
    def test_find_forbidden_fields_walks_nested_values(self):
        ...
        self.assertEqual(
            ...
            admission.find_forbidden_fields(value),
        )
        self.assertEqual([], admission.find_forbidden_fields({"decisions": []}))
    ...
    def test_input_candidate_ids_must_be_unique(self):
        ...
        with self.assertRaises(ValueError):
            admission.validate_admission_response(valid_admission_response(ids), ids)
...
class AcceptedAssetsMaterializationTests(unittest.TestCase):
    def test_only_keep_candidates_are_materialized(self):
        ...
        accepted = admission.build_accepted_assets(document, decisions)
    ...
    def test_accepted_assets_document_inherits_a1_image_metadata(self):
        ...
        accepted = admission.build_accepted_assets(document, decisions)
        self.assertEqual(admission.ADMISSION_SCHEMA_VERSION, accepted["schema_version"])
...
class ProviderUsageTests(unittest.TestCase):
    def test_usage_fields_are_extracted_from_provider_envelope(self):
        ...
        self.assertEqual(
            ...
            admission.extract_provider_usage(envelope),
        )
    ...
    def test_missing_usage_fields_become_null(self):
        self.assertEqual(
            ...
            admission.extract_provider_usage({"choices": [{"finish_reason": "stop"}]}),
        )
        self.assertEqual(
            {
                "finish_reason": None,
            ...
            admission.extract_provider_usage({"usage": {"prompt_tokens": 3252}}),
        )
        self.assertEqual(
            {
                "finish_reason": None,
            ...
            admission.extract_provider_usage(None),
        )
...
class AssetAdmissionRunTests(unittest.TestCase):
    ...
    def _run_with(self, client, runs=1, document=None, image_size=(64, 64)):
        ...
        summary = admission.run_admission(
            image,
            candidates_json,
            self.output,
            client=client,
    ...
    def test_parse_error_provider_envelope_is_saved_before_error_propagates(self):
        envelope = {
            "choices": [
                {
...
class AssetAdmissionCliTests(unittest.TestCase):
    ...
    def test_main_builds_production_client_with_model_override(self):
        ...
        self.assertEqual(
            {"max_tokens": admission.ADMISSION_MAX_TOKENS},
            client_factory.call_args.kwargs,
        )
        self.assertEqual(12000, admission.ADMISSION_MAX_TOKENS)
        run_admission.assert_called_once()
