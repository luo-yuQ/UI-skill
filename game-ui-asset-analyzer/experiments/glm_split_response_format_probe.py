"""GLM split response_format probe（仅测 Case B / Case C）。

背景（由 Control-0 实跑结论得出）：
- production vlm_client.py 调用链可真实拿到 HTTP 200 与完整 GLM provider
  envelope，但 GLM 在 OpenAI nested json_schema（Case A）下不受约束，
  返回普通自然语言图片描述。
- 旧手写 requests.post 路径已证明不可用（HTTP 204 / openresty / 空 body），
  本实验绝不复用该路径，也不退回裸 requests.post。

本实验（experiment-only，不修改任何现有文件）：
- 不调用 production client 的 infer_json(...)（其内部硬编码 Case A nested
  json_schema），而是在本文件内建立 ExperimentChatCompletionsTransport，
  逐项复制 production ChatCompletionsVLMClient 的真实 transport 行为。
- 只测试两种拆分字段形式：
  * Case B（case-b-split-json-schema）:
      payload["response_format"] = "json_schema"
      payload["response_format_schema"] = SCHEMA
  * Case C（case-c-none-plus-schema）:
      payload["response_format"] = "none"
      payload["response_format_schema"] = SCHEMA
- 最重要探针字段：mandatory_unused_field（prompt 中绝不出现；只有 schema
  真正约束了模型输出时才会出现该字段）。不预设、不硬编码任何 case 结论。

Exit code 约定：0 = 两个 case 都完成了一次真实 HTTP 交互（schema 是否有效
是被测量的实验结论，不是失败条件）；1 = 存在配置 / 图片 / payload 断言 /
transport 层面的失败。
"""
...
# 与 Control-0 完全相同的固定图片与固定 prompt。
# prompt 中不含任何 schema 字段名（schema_version / foo / number /
# mandatory_unused_field），也没有 JSON example —— schema 约束只能来自
# response_format 字段本身。
IMAGE_PATH = Path(
...
OUTPUT_ROOT = Path("runs/20260903_glm_split_response_format_probe")
SUMMARY_SCHEMA_VERSION = "glm-split-response-format-probe-v0.1"

# 与 glm_structured_output_probe.py / Control-0 完全相同的固定 SCHEMA。
SCHEMA = {
    "type": "object",
...
CASE_B_NAME = "case-b-split-json-schema"
CASE_C_NAME = "case-c-none-plus-schema"
...
def assert_no_nested_json_schema(payload: dict) -> None:
    """确认最终 payload 使用拆分字段形式，绝无 nested json_schema（Case A）。"""

    if "json_schema" in payload:
        raise AssertionError(
            "payload must not contain a nested json_schema key (Case A form)"
        )
    response_format = payload.get("response_format")
    if not isinstance(response_format, str):
        raise AssertionError("response_format must be the split string form")
    if "response_format_schema" not in payload:
        raise AssertionError("response_format_schema field must be present")


def build_case_payload(transport: ExperimentChatCompletionsTransport, case_name: str) -> dict:
    """构造最终真正发出去的 payload（基础字段 + 拆分 response_format 字段）。"""
    ...
    if case_name == CASE_B_NAME:
        payload["response_format"] = "json_schema"
        payload["response_format_schema"] = SCHEMA
    elif case_name == CASE_C_NAME:
        payload["response_format"] = "none"
        payload["response_format_schema"] = SCHEMA
    else:
        raise ValueError(f"unknown case: {case_name}")
    assert_no_nested_json_schema(payload)
    return payload
...
def run_case(
    ...
    result = {
        "case": case_name,
        "response_format": "json_schema" if case_name == CASE_B_NAME else "none",
        "http_status": None,
    ...
    print(case_name)
    print("=" * 70)
    print("RESPONSE_FORMAT =", result["response_format"], "(+ response_format_schema)")

    # 1) 构造最终 payload（拆分字段形式；断言绝无 nested json_schema）
    try:
        payload = build_case_payload(transport, case_name)
    except (AssertionError, ValueError) as exc:
        result["error"] = str(exc)
        result["error_type"] = type(exc).__name__
        print("CASE_ERROR =", result["error"])
        return result
    ...
    if response.status_code != 200:
        ...
        print("REQUEST_FAILED — details in", case_dir / "raw-http-response.txt")
        return result
    ...
    result["probe_field_present"] = PROBE_FIELD in parsed

    write_json(case_dir / "validation-result.json", validation)

    # 10) 每 case 控制台结果
    print("JSON_PARSE_OK =", result["parse_ok"])
    print("SCHEMA_VALID =", result["schema_valid"])
    print("MANDATORY_UNUSED_FIELD_PRESENT =", result["probe_field_present"])
...
def print_final(case_results: dict, summary_path: Path) -> None:
    print()
    print("=" * 70)
    print("FINAL")
    print("=" * 70)
    for name, result in case_results.items():
        print(
            ...
            "MANDATORY_UNUSED_FIELD_PRESENT=", result["probe_field_present"],
        )
...
def main() -> int:
    print("=" * 70)
    print("GLM split response_format probe (Case B / Case C only)")
    print("=" * 70)
    ...
    try:
        # 与 asset_admission_probe.py（A2）/ Control-0 完全一致的配置链：
        # base URL / API key 来自 STAGE2A_VLM_* env vars，model 固定为
    ...
    case_results = {}
    for case_name in (CASE_B_NAME, CASE_C_NAME):
        case_results[case_name] = run_case(
            transport, case_name, OUTPUT_ROOT / case_name
        )

    summary_path = write_summary(OUTPUT_ROOT, config, transport, case_results)
    print_final(case_results, summary_path)

    return 0 if all(r["request_completed"] for r in case_results.values()) else 1
